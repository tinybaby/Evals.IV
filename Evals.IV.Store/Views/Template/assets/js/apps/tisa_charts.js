		...
		init: function() {
				...
				function defaultChartConfig(containerid, data, guideline, useDates, auxOptions) {
					...
					nv.addGraph(function() {
						...
						if (useDates !== undefined) {
							...
							formatter = d3.format("http://tisa-admin.tzdthemes.com/assets/js/apps/,.1f");
						}
						...
						chart.yAxis
							.tickFormat(d3.format('http://tisa-admin.tzdthemes.com/assets/js/apps/,.2f'));
			...
			if ($('#nvd3_live').length) {
				...
				nv.addGraph(function() {
					...
					chart.xAxis
						.tickFormat(d3.format('http://tisa-admin.tzdthemes.com/assets/js/apps/,.1f'))
						.axisLabel("Time");
				  
					chart.yAxis
						.axisLabel('Random Number')
						.tickFormat(d3.format('http://tisa-admin.tzdthemes.com/assets/js/apps/,.4f'));
		...
		init: function() {
			...
			if($('#flot_tracking').length) {
				...
				var plot = $.plot("#flot_tracking", [
					{ data: sin, label: "http://tisa-admin.tzdthemes.com/assets/js/apps/sin(x) = -0.00"},
					{ data: cos, label: "http://tisa-admin.tzdthemes.com/assets/js/apps/cos(x) = -0.00" }
				], {
					series: {
						lines: {
							show: true
